...
- A varredura de dado pessoal procura o valor inteiro (telefone com os 11 dígitos), nunca um trecho de 4 ou 5 dígitos: o dump concatena UUID e hash, onde a sequência curta aparece por acaso e o teste falha em um push aleatório. O par de mutações é "não limpar o campo" (morre) e "limpar" (sobrevive).
...
- O drizzle-kit grava o snapshot novo e o `_journal.json` fora do formato do Biome, e o `pnpm check` reprova a migration recém-gerada: rode `pnpm exec biome check --write` nos dois logo depois do `generate`.
- Coluna nova numa tabela que já tem linha em produção (a instalação) se prova com a linha existente: copie `src/migrations` para um diretório temporário com o `_journal.json` cortado antes da migration nova, aplique o `migrate` do Drizzle nele, insira a linha e rode o `applyMigrations` completo (`packages/db/tests/services-schema.test.ts`). O migrator aplica só o que vem depois da última migration registrada. A prova de que a tabela não foi recriada compara o `sql` do `sqlite_master`: o `ADD COLUMN` do SQLite insere a definição antes das restrições de tabela (`FOREIGN KEY`, `CONSTRAINT`), então tirar essa definição devolve o texto de antes; a recriação troca o cabeçalho para `CREATE TABLE "installation"` e reordena as colunas, mesmo preservando dados e `CHECK`.
- Migration editada depois de aplicada não roda de novo: o migrator do Drizzle compara só o `created_at` da última migration registrada com o `when` do `_journal.json`, e o banco segue com o SQL velho sem aviso. Migration da entrega muda até o passo "Dados de exemplo" migrar o `local.db` do dono; a cópia de ensaio migrada antes da mudança é refeita.
