...
- Custo médio de um ponto só existe com quantidade positiva e valor não negativo: ponto com valor negativo (sobra de um negativo sem ajuste de custo) daria saída com valor negativo. `consumptionPartValue` trata esse ponto como sem média, e a saída inteira vira provisória pela referência; `exitValueCents` devolve zero para ele. O brinde a R$ 0 tem média zero, e tratar o zero como sem média inventava custo pela referência.
- Sequência de saldo de várias saídas numa operação (`valueConsumptionParts`) mora no domínio e é chamada pelo servidor e pela prévia da tela; prévia calculada linha a linha divergia do servidor quando a troca punha a mesma variante em duas linhas.
- Plano de reserva de várias necessidades (`planReservations`) acumula por variante na ordem recebida e devolve o reservado e a falta de cada uma; servidor e prévia chamam a mesma função, e a ordem (subitens na ordem das linhas, variantes na ordem de primeira aparição por `linePlannedMaterials`) é parte do contrato.
