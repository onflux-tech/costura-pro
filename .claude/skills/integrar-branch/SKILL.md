...
1. **Entrega fechada.** Confirme na branch que `/entrega-fechar` rodou, que `pnpm harness:check`, `pnpm docs:check` e `/verificar` estão verdes e que a árvore está limpa: os commits da entrega já existem, um por checkpoint, correção e área do fechamento, e aqui não se monta nem se reescreve commit. Pronto com a evidência no chat.
