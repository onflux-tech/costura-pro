...
type Stock = { crepeId: string; locationId: string; zipperId: string };
...
async function seedStock(owner: Owner): Promise<Stock> {
...
type Person = { profileId: string; receivedItemId: string };

async function seedPerson(owner: Owner, clientId: string): Promise<Person> {
...
function serviceLine(person: Person) {
	return {
...
function pieceLine(stock: Stock, profileId: string | null) {
	return {
		components: [
...
function itemsOf(server: TestServer, serviceOrderId: string) {
	return server
		.native()
...
describe("quote approval", () => {
	...
	test("numbers the codes by the year of the acceptance", async () => {
		...
		const { owner, server } = await ownerSetup({ now: clock.now });
		const clientId = await createClient(owner);
...
describe("service order reads", () => {
	async function readSetup() {
		...
		const setup = await ownerSetup({ now: clock.now });
		const stock = await seedStock(setup.owner);
		const clientId = await createClient(setup.owner);
		const person = await seedPerson(setup.owner, clientId);
