...
| Sessão de inventário, contagem, rascunho no aparelho ou movimento `inventory` | [Estoque](areas/estoque.md), [ADR 0022](adr/0022-sessao-de-inventario-como-fato-com-esperado-da-contagem.md), [ADR 0018](adr/0018-movimento-append-only-com-projecao-de-saldo.md), [Agregados](areas/agregados.md), [SPEC §2 a §4](SPEC.md#2-persistência-valores-e-fronteiras-de-domínio), rules `.claude/rules/domain.md`, `.claude/rules/db.md`, `.claude/rules/server.md` e `.claude/rules/web.md` |
| Produto, variante de produto, galeria, capa ou ficha técnica | [Produtos](areas/produtos.md), [ADR 0021](adr/0021-ficha-tecnica-no-produto-base-e-ajustes-por-variante.md), [Serviços](areas/servicos.md), [Mídia](areas/midia.md), [Agregados](areas/agregados.md), [SPEC §2 a §4](SPEC.md#2-persistência-valores-e-fronteiras-de-domínio), rules `.claude/rules/domain.md`, `.claude/rules/db.md`, `.claude/rules/server.md` e `.claude/rules/web.md` |
| Busca global (diálogo ou página), filtro de busca de uma lista, normalização de texto, trecho marcado, paleta de busca, abas ou atalho de busca | [Busca](areas/busca.md), [Design system](areas/design-system.md), [SPEC §4](SPEC.md#4-api-sincronização-e-conflito), [Agregados](areas/agregados.md#redação-de-dado-pessoal), rules `.claude/rules/domain.md`, `.claude/rules/server.md` e `.claude/rules/web.md` |
| Orçamento, linha do orçamento, peça sob medida, desconto, emissão de revisão, recusa ou estado do orçamento | [Orçamentos](areas/orcamentos.md), [ADR 0023](adr/0023-orcamento-rascunho-com-revisao-emitida-como-fato.md), [ADR 0009](adr/0009-uuid-e-codigo-documental-por-dispositivo.md), [Produtos](areas/produtos.md), [Agregados](areas/agregados.md), [SPEC §2 a §4](SPEC.md#2-persistência-valores-e-fronteiras-de-domínio), rules `.claude/rules/domain.md`, `.claude/rules/db.md`, `.claude/rules/server.md` e `.claude/rules/web.md` |
...
| Roteiro de navegador grava acento deformado e o app parece quebrado | O heredoc do browser-harness chega ao Python com o encoding do console do Windows, e o texto sai trocado antes de virar `Input.insertText` ou corpo de `fetch` | Rodar o browser-harness com `PYTHONUTF8=1`, que lê o roteiro em UTF-8; sem ele, comparar por trecho ASCII e escapar o acento em \uXXXX dentro do JavaScript. O app não tem esse problema: a suíte do servidor grava o nome acentuado e passa |
...
| Texto com acento some, não é encontrado ou é gravado corrompido ("AteliÃª") num roteiro do browser-harness, ou o roteiro para com `UnicodeEncodeError` ("surrogates not allowed") | O browser-harness lê o stdin com a página de código do Windows (cp1252): acento vira dois caracteres, e aspa tipográfica ou seta vira surrogate | Rodar com `PYTHONUTF8=1` (`PYTHONUTF8=1 BU_NAME=... browser-harness < roteiro.py`), que lê o roteiro e escreve a saída em UTF-8; localizar por estrutura (`form label`, `data-slot`) quando der |
...
| Roteiro do browser-harness não acha mais nenhum campo depois de preencher uma categoria | O `SuggestionField` (combobox do Base UI) deixa a lista de sugestões aberta, e o resto da página fica inerte na árvore de acessibilidade | Esc depois de preencher campo com sugestões (rule de web) |
| Roteiro Python do browser-harness quebra com `unicodeescape` ou grava caminho sem as barras | O heredoc do Git Bash colapsa a barra invertida dupla em uma só antes do Python ler o texto | Roteiro escrito num arquivo pelo editor, nunca por heredoc, com string crua ou barras normais nos caminhos (rule de web) |
| Enter enviado pelo CDP não aciona o botão focado | `Input.dispatchKeyEvent` sem `code` e `text` não vira clique no botão nativo | `keyDown` com `code: "Enter"` e o texto do retorno de carro (rule de web) |
| `bun test` verde e `pnpm check-types` vermelho no mesmo arquivo de teste da web | O Bun transpila sem checar tipos; os testes da web só passam pelo `tsc -p tests` do `check-types` | `pnpm --filter web check-types` junto com os testes em todo passo que mexe em teste da web (rule de web) |
| `pnpm check` reprova a migration que acabou de ser gerada | O drizzle-kit grava o snapshot e o `_journal.json` fora do formato do Biome | `pnpm exec biome check --write` no snapshot novo e no journal logo depois do `generate` (rule de banco) |
| O TypeScript diz que a propriedade não existe depois de um `if` sobre `kind` | Membro de união cujo `kind` é ele mesmo uma união de literais não é estreitado pelo `if` | Um membro da união por valor de `kind` (rule de web) |
| Tela mostra um registro e grava em outro depois de voltar pelo histórico | A rota com o mesmo componente não remonta quando só o parâmetro muda, e a página congelada mistura o dado aberto com o parâmetro da URL | `key` pelo parâmetro na rota da página congelada (rule de web) |
...
| Fotos do banco de desenvolvimento somem do disco depois de um pull ou fast-forward | O commit que tirou `media/` do rastreio com `git rm --cached` apaga o arquivo em todo clone que o recebe | Backup antes e `git restore --source=<commit anterior> --worktree -- media` (rule de harness) |
| Contagem soma duas vezes a compra lançada noutra janela antes de contar | O esperado vinha do cache da lista, que o TanStack Query só renova por foco, montagem ou reconexão | `refetchInterval` enquanto a tela captura e ação liberada só com `isFetchedAfterMount` ([estoque](areas/estoque.md)) |
| Sobra recontada entra pelo valor digitado para a quantidade antiga | O valor digitado sobrevivia à mudança da quantidade que ele valoriza | Valor apagado quando a base muda e dica que separa digitado de sugestão |
| Alerta de erro não recebe o foco no primeiro clique | `focus()` chamado no handler, antes do render que monta o alerta | Alvo guardado em estado e focado num `useEffect` |
| Leitor de tela anuncia dois campos com o mesmo nome na contagem | Rótulo só com material e variante, sem o lote nem o local | `pointLabel` no rótulo |
| Texto de apoio sai maior que o corpo | `Text size="sm"` é o `text-sm` do Tailwind (14 px), e o corpo usa `text-md` (13 px) | `xs` para texto menor que o corpo |
| Roteiro do browser-harness para com "No target with given id found" | O `new_tab` reaproveitou a aba `about:blank`, e a limpeza das abas `about:blank` fechou a aba de trabalho | Conferir `current_tab()` antes de fechar qualquer aba |
| Servidor de verificação abre o `local.db` do dono | Sem `DATABASE_FILE`, o processo lê o `.env` do `apps/server`, que aponta para o banco de desenvolvimento | `DATABASE_FILE` explícito no scratchpad e `installation.status` vazio conferido antes de gravar |
| `pnpm check` reprova a ordem de import de arquivo editado por script | Edição por Node ou `sed` não passa pelo hook de formatação | `pnpm exec biome check --write` nos arquivos tocados depois do script |
| Texto entre crases some de um arquivo editado por `node -e` | O Bash executa a crase dentro das aspas duplas como substituição de comando | Script num arquivo `.cjs` escrito pelo editor |
| A busca global e a lista da área acham coisas diferentes para a mesma palavra | Busca e lista com filtro ou ordem próprios | Uma função de filtro por área chamada pelas duas, e teste que compara os primeiros ([busca](areas/busca.md)) |
| Perfil com acento não aparece na busca | `client_profile` não tem `search_text`, e `lower()` do SQLite só converte ASCII | Casar em memória com `matchesAll` do domínio (rule de servidor) |
| Mutação na ordem da consulta sobrevive com o teste verde | A troca da expressão por `sql\`1\`` vira `ORDER BY 1`, a primeira coluna do select (a data de arquivamento), e o teste tinha um arquivado só por grupo, onde as duas ordens coincidem | O teste de ordem tem registros que a ordem errada inverteria: dois arquivados em ordem contrária à alfabética, pelo `manualClock` (rule de servidor, papel `reviewer`) |
| `@costura-pro/domain/<modulo>` não resolve com o arquivo no lugar | Módulo novo do domínio sem entrada no `exports` do `packages/domain/package.json` | Entrada com `types` no `dist` e `default` no `src` para cada módulo (rule de domínio) |
| Roteiro do browser-harness não acha o campo de busca | `input type="search"` tem o papel `searchbox` na árvore de acessibilidade | Procurar por `searchbox`, como `Date` e `radio` (rule de web) |
| Foco no checkbox aparece como "SPAN" num roteiro | O `Checkbox` do Base UI é um `span` sem texto, e o texto do elemento ativo vem vazio | Conferir o nó com `focused` na árvore de acessibilidade (rule de web) |
| Teclas digitadas na busca somem quando a leitura falha | Alerta de erro que recebe o foco no meio da digitação | Na busca por digitação, o alerta não rouba o foco; o `role="alert"` anuncia (rule de web) |
| Verificação de falha de rede não vê o alerta de erro | O `QueryClient` tenta a leitura mais 3 vezes antes do erro, e o roteiro derrubou só a primeira | Derrubar toda `Fetch.requestPaused` num laço até o alerta surgir (rule de web) |
| Semeio que anonimiza um cliente falha na segunda rodada com "opId reutilizado com conteúdo diferente" | A anonimização redige o hash das operações do cliente, então repetir a criação pelo mesmo `opId` não bate mais | O roteiro de semeio confere se o cliente já está anonimizado e pula a criação e a anonimização |
| Campo da paleta de busca abre com uma caixa alta e o anel cortado no topo do diálogo | Campo sem borda que ocupa a linha inteira dentro de um contêiner com `overflow-hidden`: campo de texto casa com `:focus-visible` em qualquer foco, então o anel aparece ao abrir, e o popup corta o que passa da borda | O foco do campo aparece na linha, por um traço inset com `has-[input:focus-visible]`, e o `focus-ring.test.ts` aceita esse par só na paleta (rule de web) |
| O trecho casado some justo no resultado em destaque | A marca e a linha em destaque usam o mesmo `bg-accent` | Dentro de `data-highlighted`, a marca passa a `nav-active` (rule de web) |
| Espaço vazio embaixo dos resultados da paleta | O `Autocomplete.Empty` do Base UI fica sempre montado como região viva, e o espaçamento dele ocupa a lista mesmo sem texto | `empty:py-0`; esconder com `display: none` quebraria o anúncio (rule de web) |
| O teste de anel exige anel claro num componente fora da barra verde | A regra casava `\bbg-nav\b`, e `bg-nav-active` também casa | A regra casa só `bg-nav` exato; um mutante com anel escuro no `mobile-header.tsx` ainda reprova |
| O campo da página de busca volta ao texto antigo depois do "Ver todos" do diálogo | A espera de 300 ms comparava o texto digitado com a URL nova e gravava o texto velho por cima | Um ref guarda o último valor que a página gravou, e mudança de fora substitui o texto ([busca](areas/busca.md)) |
| Tela parada, sem erro nem resultado, com o navegador sem internet no acesso local | O `networkMode` padrão `online` do TanStack Query pausa a leitura quando o navegador se diz sem rede, mesmo com o servidor no mesmo computador | A busca usa `networkMode: "always"`; o resto do app está pendente no ROADMAP (F3) |
| A verificação no navegador mostra a tela de antes do build | O service worker da PWA no Chrome da verificação serve o precache antigo | `Storage.clearDataForOrigin` com `service_workers,cache_storage` no começo do roteiro, que mantém os cookies (rule de web) |
| Salvar uma linha ou tirar um item do orçamento volta "Confira os campos e tente de novo." | O desconto do orçamento passaria do subtotal, e o schema recusa o conteúdo inteiro sem apontar campo | A tela confere `documentDiscountFits` antes de mandar e explica o motivo ([orçamentos](areas/orcamentos.md)) |
| Nova tentativa depois de resposta perdida na emissão grava revisão duplicada, ou a emissão seguinte diz "Revisão N emitida" sem gravar | Id da revisão e `opId` sorteados de novo a cada clique; ou o id guardado reaproveitado depois que a revisão dele já existe | Rascunho da página por `useDrafts` (chave `emitir`), descartado no sucesso e quando a revisão dele aparece na lista; "Registro já existe" vira aviso, nunca sucesso ([orçamentos](areas/orcamentos.md)) |
| Revisão redigida na anonimização não chega ao aparelho que já a tinha puxado | A redação reescrevia só o `change_log` antigo, no mesmo cursor, e a trigger travava a versão | A redação da revisão sobe a versão uma vez e grava mudança nova no `change_log` (`redactQuoteRevision`), com teste de pull a partir do cursor da emissão |
| Código do orçamento cortado no celular ("ORC-2026-PC-00...") | O `MobileHeader` trunca o título a 320 px, e o `Heading` da página era só para leitor de tela abaixo de 768 px | O código fica visível no painel do cabeçalho em toda largura (rule de web) |
| Roteiro do browser-harness não acha o rótulo de grupo que está na tela | `innerText` devolve o texto com o `text-transform` do CSS ("ORÇAMENTOS · 2") | Comparar por `textContent` quando o rótulo é estilizado em maiúsculas (rule de web) |
| Roteiro do browser-harness não acha o botão da página com o `AlertDialog` aberto | O diálogo deixa o resto da página inerte e fora da árvore de acessibilidade, e o único botão com aquele nome é o do diálogo | Procurar o botão do diálogo pelo índice 0 enquanto ele está aberto (rule de web) |
| `vite build`, `tsc` ou o plugin da PWA caem com "memory allocation of N bytes failed", "Zone Allocation failed" ou "Worker terminated due to reaching memory limit" | Memória virtual da máquina no limite (sem arquivo de paginação além da RAM), tomada por servidores de desenvolvimento de outros projetos e pelo `tsserver` do editor | Conferir `FreeVirtualMemory` do `Win32_OperatingSystem` antes de culpar o código, repetir quando houver folga e nunca derrubar processo do dono (rule de harness) |
| Criar orçamento de novo depois de uma resposta perdida e da troca de cliente abre o orçamento do primeiro cliente | O id do orçamento era sorteado uma vez por página, e "Registro já existe" do mesmo id contava como criado | Id e `opId` num rascunho por cliente (`useDrafts`), e "Registro já existe" vira o aviso "Este orçamento já tinha sido criado" ([orçamentos](areas/orcamentos.md)) |
| Revisão emitida abaixo da meta sem a confirmação de abaixo da meta | Os avisos da emissão usavam a meta padrão enquanto a do ateliê não tinha carregado, e a revisão gravava a meta real | "Emitir" fica desativado com o motivo até a meta chegar, e o painel interno diz quando a leitura falhou (rule de web) |
| `pnpm check` reprova um arquivo inteiro que só teve uma linha mudada | Script Python regravou o arquivo com `Path.write_text`, que no Windows troca LF por CRLF | Gravar com `write_bytes` ou `newline="\n"` e procurar `\r` nos arquivos tocados (rule de harness) |
| Semeio que edita um registro falha na segunda rodada com "opId reutilizado com conteúdo diferente" | O roteiro lia a `baseVersion` do banco, e na segunda rodada a versão já subiu, então o mesmo `opId` leva outro conteúdo | Fixar no roteiro a versão que o registro tem na primeira rodada (rule de harness) |
| Banco migrado sem a mudança feita numa migration | Migration editada depois de aplicada não roda de novo: o migrator do Drizzle compara só o `created_at` da última migration registrada | Mudar migration da entrega só antes de ela chegar ao `local.db` do dono e refazer a cópia de ensaio (rule de banco) |
...
| 2026-09-23 | Passo "Dados de exemplo" no `/entrega-fechar` (migrar o `local.db` com backup e semear pelas procedures) e `/media/` no `.gitignore`, com as fotos do banco de desenvolvimento fora do índice; rule de harness e §8 com o Biome lendo `.ts` fora do git e com o `git rm --cached` que apaga o arquivo em todo clone | Pedido do dono depois da entrega de produto e ficha técnica |
| 2026-09-23 | Rules de web (saldo fresco para valor gravado, valor digitado apagado com a base, foco depois do render, rótulo com o que distingue itens repetidos, rascunho no `localStorage` por `createDraftStore`, `Text size="sm"` maior que o corpo, `DATABASE_FILE` explícito na verificação, aba `about:blank` reaproveitada pelo `new_tab` e alias `@/` no `tsconfig` dos testes da web) e de harness (edição por script sem o hook de formatação, crase em `node -e`, arquivo novo no diff da revisão por `--no-index`); §8 com essas falhas; passo 1 do `/revisar` com arquivo novo por `--no-index`; lentes no `reviewer` (valor gravado de cache velho, valor digitado que sobrevive à base, teste de recusa com mais de um motivo) e no `contract` (teto de entrada de leitura, dado com duas fontes na tela) vindas da revisão | Revisão e fechamento da entrega de sessão de inventário |
