...
| DEC-165 | Implementação por subagente: o papel `implementer` (Opus 5.5 em esforço high, com escrita) executa os checkpoints de um plano aprovado, um por vez, numa sessão de execução separada da de design e plano; `explorer`, `reviewer` e `contract` seguem somente leitura, com `reviewer` e `contract` em esforço max; nunca dois implementer ao mesmo tempo e nenhum outro teto de subagentes (2026-09-24). Substituiu o limite de 2 subagentes somente leitura por tarefa | Execução inline; fluxo completo do superpowers com revisor por tarefa; Sonnet 5 no implementer | Dupla | [HARNESS §4](HARNESS.md#4-ciclo-de-entrega-e-evolução-do-harness) |
...
| Q-02 | Etapas padrão do fluxo de produção, modelos iniciais de medidas e categorias iniciais de material e despesa | Modelos iniciais de medidas resolvidos em 2026-09-17 (DEC-78) e categorias iniciais de material em 2026-09-17 (DEC-95, como sugestões de texto livre); etapas padrão antes do fluxo de produção da F4 e categorias de despesa antes das finanças |
