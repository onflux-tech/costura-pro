...
| DEC-181 | Consumo aceita saldo negativo: a parte coberta sai pela média do ponto, e o excesso vale a média do ponto quando ele ainda tinha saldo, senão o custo de referência da variante, senão zero ("sem custo"); ponto com quantidade positiva e valor zero ou negativo (sobra de um negativo sem o ajuste de custo) não tem média, e a saída inteira é provisória pela referência, então nenhuma saída grava valor negativo; cada parte guarda a quantidade e o valor provisórios para o ajuste de custo, que chega com a lista de compras (2026-09-25) | Ajuste de custo na compra já agora; recusar consumo acima do saldo | Dupla | RF-EST-15, DEC-102 |
