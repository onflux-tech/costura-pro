...
| [Estoque](areas/estoque.md) | Antes de mexer em local, lote, movimento, saldo ou sessão de inventário: agregados e projeção, tipos de movimento (inclusive o consumo da reconciliação), valor de saída pela média, lote por variante, estorno, contagem com rascunho no aparelho e armadilhas |
...
| [Produção](areas/producao.md) | Antes de mexer no fluxo de produção, nas etapas sugeridas do serviço, na produção do subitem, no quadro por etapa ou na reconciliação de materiais (consumo, custo provisório, troca, estorno e reserva liberada): onde mora cada coisa, regras do domínio com o exemplo, comandos e leituras, telas e armadilhas |
