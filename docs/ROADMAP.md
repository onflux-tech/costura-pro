...
- Sessão de inventário (2026-09-23), descrita em [estoque](areas/estoque.md):
  - contagem física às cegas dos locais escolhidos, num rascunho guardado no aparelho, finalizada de uma vez como fato imutável com o motivo, a data e as linhas contadas, inclusive as que bateram ([DEC-122 a DEC-128](PRD.md#93-catálogo-estoque-e-produção), [ADR 0022](adr/0022-sessao-de-inventario-como-fato-com-esperado-da-contagem.md));
  - esperado de cada linha capturado na hora da contagem, com o ajuste somado ao saldo da finalização, e aviso na revisão quando o saldo mudou depois da contagem; ponto não contado fica como está, com "Contar como zero" na revisão;
  - lista dos locais renovada a cada 15 s durante a contagem, até 100 locais por contagem, valor de sobra digitado apagado quando o contado muda, contagens fora do formato nomeadas no alerta da finalização e lote no nome de cada campo, vindos da revisão da entrega e conferidos de novo no navegador;
  - movimento do tipo `inventory` ligado à sessão, estornável um a um, com a falta pela média do ponto e a sobra pelo valor informado, sugerido pela média do ponto ou pelo custo de referência;
...
- Fluxo de produção versionado, etapas sugeridas no serviço, produção por subitem e quadro por etapa (2026-09-25), descritos em [produção](areas/producao.md):
  - fluxo semeado no boot com Corte, Montagem, Prova e Acabamento, editado na sub-aba Fluxo de produção (renomear, subir e descer, ocultar, mostrar e adicionar, de 1 a 12 etapas com nome de até 30 caracteres sem repetição por acento, caixa ou espaço), cada gravação gerando versão nova e a etapa oculta ficando na lista ([DEC-168, DEC-171, DEC-175, DEC-176](PRD.md#93-catálogo-estoque-e-produção), [ADR 0025](adr/0025-fluxo-de-producao-como-agregado-com-copia-na-os.md));
  - a OS nasce com a cópia do fluxo vigente e troca de versão só pelo "Usar a versão nova"; a OS aprovada antes do fluxo oferece "Usar o fluxo v<N>" ([DEC-172](PRD.md#93-catálogo-estoque-e-produção));
  - serviço com etapas sugeridas, que pré-marcam o "Iniciar produção" do subitem (a peça une as dos serviços dela); cada subitem de serviço ou peça inicia com as etapas aplicáveis, avança e volta sozinho, e vai a "Pronto" só sem material planejado ([DEC-170, DEC-173](PRD.md#93-catálogo-estoque-e-produção));
  - trilha, etapa atual e ações no cartão do subitem, faixa "Produção" com atrasados e bloqueio por falta de material, resumo na lista de OS, e quadro por etapa no destino Produção, com colunas no desktop e abas abaixo de 1280 px ([DEC-169, DEC-174](PRD.md#93-catálogo-estoque-e-produção));
  - verificado em navegador real no build de produção com banco novo: exemplo completo com o serviço até "Pronto" e de volta, a peça parada em Acabamento com o aviso da reconciliação e "bloqueado · falta Crepe · Preto", o material com "Só entrega, sem produção", a faixa "em produção · 1 atrasado · bloqueado" e a lista com o resumo; fluxo editado para v2 (Montagem renomeada para Costura, Prova oculta, Passadoria no fim) com a OS em v1 até "Usar a versão nova", a OS nova nascendo em v2 e a coluna Prova no quadro só enquanto teve cartão; OS antiga com "Usar o fluxo v1"; erros de nome vazio, longo e repetido por acento, caixa e espaço, teto de 12 e "Mantenha ao menos uma etapa"; conflito do editor com "Carregar versão atual"; resposta perdida no avanço seguida de nova tentativa sem andar duas vezes e segunda janela recebendo "Este subitem mudou em outra janela. Confira e tente de novo."; 1440, 1280, 1024, 768, 390 e 320 px sem rolagem horizontal no quadro, na OS, no fluxo, no serviço e no diálogo, toque a 390 px e teclado com foco preso no diálogo e Esc devolvendo o foco.

- Consumo com lote sugerido e reconciliação de materiais (2026-09-25), descritos em [produção](areas/producao.md) e [estoque](areas/estoque.md):
  - na última etapa, a peça com material planejado vai a "Pronto" pela reconciliação, numa operação só: consumido e perdido por material, saídas por local e lote com o lote mais antigo sugerido e "Dividir", troca de material com a mesma unidade e motivo, e a sobra ficando no estoque sem movimento ([DEC-179 a DEC-187](PRD.md#93-catálogo-estoque-e-produção), [ADR 0026](adr/0026-reconciliacao-como-fato-que-consome-e-libera-a-reserva.md));
  - fato append-only com linhas e partes, um movimento `consumption` por parte e saldo negativo com custo provisório pela média do ponto, pela referência ou sem custo, guardado na parte para o ajuste de custo;
  - reserva do subitem reconciliado fora do reservado (aprovação, Saldos, ficha do material, detalhe da variante e materiais previstos) e da falta; "Voltar" mantém a reconciliação; estorno da reconciliação inteira, que devolve o estoque pelo mesmo valor, faz a reserva voltar a contar e tira a peça de "Pronto"; estorno avulso do consumo recusado;
  - diálogo "Reconciliação de materiais" na OS e no quadro, tabela Previsto, Consumido, Perdido e Sobra no cartão, selo "reconciliado", material previsto contra o real no painel "Só para você" e "Consumo" com "Ver OS" no histórico do estoque;
  - verificado em navegador real no build de produção com banco novo: crepe ficando em −0,90 m com provisório de R$ 30,00/m pela média, provisório pela referência, troca com motivo e a variante de outra unidade desativada, "Dividir" com o lote mais antigo sugerido, sobra e "a mais", "Pronto", "Voltar" e avanço direto a "Pronto", estorno, resposta perdida seguida de nova tentativa sem duplicar, segunda janela com o aviso, reconciliação pelo quadro com o selo, histórico com "Ver OS", Saldos sem a reserva liberada, 1440, 1280, 768, 390 e 320 px sem rolagem horizontal, toque a 390 px e teclado com foco preso e Esc devolvendo o foco.
