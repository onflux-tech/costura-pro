...
type Operation = {
  ...
  aggregateType: string; // "installation" | "device" | "client" | "profile" | "measurementTemplate" | "measurement" | "receivedItem" | "material" | "materialVariant"
  aggregateId: string;
  baseVersion: number | null; // null obrigatório na criação
  occurredAt: string; // ISO 8601 com fuso
  command: string; // "installation.setAtelierName" | "device.rename" | "client.*" | "profile.*" | "measurementTemplate.*" | "measurement.*" | "receivedItem.*" | "material.*" | "materialVariant.*"
  payload: unknown;
};
