...
| Catálogo, estoque e compras | Previsto | | F3 |
...
**Estado:** cliente, perfil, modelo de medidas, medição e peça recebida implementados; o resto previsto (F3 a F5).
...
type Operation = {
  ...
  aggregateType: string; // "installation" | "device" | "client" | "profile" | "measurementTemplate" | "measurement" | "receivedItem"
  aggregateId: string;
  baseVersion: number | null; // null obrigatório na criação
  occurredAt: string; // ISO 8601 com fuso
  command: string; // "installation.setAtelierName" | "device.rename" | "client.*" | "profile.*" | "measurementTemplate.*" | "measurement.*" | "receivedItem.*"
  payload: unknown;
};
