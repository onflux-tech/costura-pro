...
- **Estado** (`quoteStatus`). Derivado, nunca gravado: recusado com `refused_on`; rascunho sem revisão; vencido quando o `valid_until` da revisão de maior número é anterior ao dia de hoje da tela; emitido nos demais casos.
...
**Estado:** cliente, perfil, modelo de medidas, medição, peça recebida, material, variante, local, lote, movimento de estoque, fornecedor, compra, obrigação, conta, movimento financeiro, serviço, produto, variante de produto, sessão de inventário, orçamento e revisão do orçamento implementados; aprovação, OS e o resto previstos (F4 e F5).
...
type Operation = {
  ...
  aggregateType: string; // "installation" | "device" | "client" | "profile" | "measurementTemplate" | "measurement" | "receivedItem" | "material" | "materialVariant" | "stockLocation" | "stockLot" | "stockMovement" | "supplier" | "purchase" | "purchaseReversal" | "obligation" | "financialAccount" | "financialMovement" | "service" | "product" | "productVariant" | "inventorySession" | "quote" | "quoteRevision"
  aggregateId: string;
...
| Procedure | Comando | Entrada | Resposta |
|---|---|---|---|
| `quotes.list` | leitura | `{ status, today, query?, archived = false, offset = 0 }`, `status` entre `draft`, `emitted`, `expired` e `refused`, `today` em `AAAA-MM-DD` | `{ items: { id, code, clientId, clientName, createdOn, emittedOn, validUntil, revisionNumber, refusedOn, lineCount, totalCents, status, archivedAt }[], nextOffset }`, 50 por página, ordem por `created_at` e `id` decrescentes; `totalCents` é o da última revisão ou, no rascunho, o do conteúdo atual; `query` casa o `search_text` do orçamento ou o do cliente |
| `quotes.get` | leitura | `{ quoteId }` | `{ client: { id, name, archived, anonymized }, quote: snapshot com updatedAt, revisions: snapshot[] em número decrescente, stock: { variantId, quantityMicros }[] }`, com o saldo físico somado de cada variante dos materiais previstos (`"0"` sem ponto) |
...
| Procedure | Comando | Entrada | Resposta |
|---|---|---|---|
| `search.global` | leitura | `{ query, archived = false }`, `query` de 2 a 100 caracteres depois do trim | `{ clients, profiles, quotes, products, materials, services }`, cada grupo `{ items, total }` com até 5 itens e o total do grupo |
| `search.group` | leitura | `{ query, archived = false, group, offset = 0 }`, `group` entre `clients`, `profiles`, `quotes`, `products`, `materials` e `services`, `offset` inteiro não negativo | `{ group, items, nextOffset, total }`, com até 50 itens do grupo a partir de `offset`, `nextOffset` nulo na última página e o mesmo item de `search.global` |
...
| `quotes` | `quoteMatches`, o filtro da lista: `search_text` do orçamento (código, dígitos do código e títulos das linhas) ou o do cliente; nunca orçamento de cliente anonimizado; ativos primeiro e depois `created_at` e `id` decrescentes | `{ id, code, clientName, refused, revisionNumber, validUntil, totalCents, archived }`, com o estado calculado na tela pelo dia de hoje |
