...
| Peça recebida, catálogo, estoque e compras | Previsto | | F3 |
...
**Estado:** cliente, perfil, modelo de medidas e medição implementados; o resto previsto (F3 a F5).
...
type Operation = {
  ...
  aggregateType: string; // "installation" | "device" | "client" | "profile" | "measurementTemplate" | "measurement"
  aggregateId: string;
  baseVersion: number | null; // null obrigatório na criação
  occurredAt: string; // ISO 8601 com fuso
  command: string; // "installation.setAtelierName" | "device.rename" | "client.*" | "profile.*" | "measurementTemplate.*" | "measurement.*"
  payload: unknown;
