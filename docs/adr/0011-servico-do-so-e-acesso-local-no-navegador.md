...
- Backup, restauração, Tunnel, atualização e aprovação de dispositivo são ações do **acesso local**: Host de loopback, sem `cf-connecting-ip` e com sessão do dono. Qualquer processo do PC alcança o loopback, como já acontecia com a janela do Tauri, então sessão e código de recuperação continuam obrigatórios onde a SPEC pede. Sem sessão, e ainda só no acesso local, ficam apenas os passos anteriores à conta do dono (nome do ateliê e criação do dono) e o reset de senha, em que o código de recuperação é a credencial ([SPEC §1](../SPEC.md#1-topologia-e-componentes)).
