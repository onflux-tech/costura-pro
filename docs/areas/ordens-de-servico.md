...
Aprovação, reserva e recebível são fatos append-only; OS e subitem são agregados comuns no molde de [agregados](agregados.md), com dado pessoal (o cliente), porque produção, entrega e reconciliação mudam cada subitem sozinho. A produção por subitem, a cópia do fluxo na OS e o resumo de produção estão em [produção](producao.md); revisão comercial, cancelamento, consumo, entrega, parcelas e pagamentos chegam nas entregas seguintes.

## Agregados e fatos

| Onde | Tipo | Campos |
|---|---|---|
| `service_order` | agregado `serviceOrder` | `client_id`, `quote_id` (único), `code` (único, `OS-<ano>-PC-<número de 4 dígitos>`), `code_year`, `code_device`, `code_number` (único por ano, sigla e número), `opened_on` (a data do aceite), `search_text` (código e títulos dos subitens), `flow_version` e `flow_stages` (cópia do fluxo de produção vigente, nulos na OS anterior ao fluxo), `version` |
...
| `service_order_item` | agregado `serviceOrderItem` | `service_order_id`, `line_id` (único por OS), `position` (a partir de 0, na ordem das linhas), `kind` (`service`, `custom`, `material`), `line` (a linha congelada inteira da revisão), `measurements` (snapshot, até 50), `due_on` (prazo combinado ou nulo), `production_status`, `stage_ids` e `stage_id` (produção do subitem), `version` |
...
As triggers `*_no_update` e `*_no_delete` da migration `0019` recusam mudar ou apagar aprovação, reserva e recebível com "`<tabela>` é append-only". A OS e o subitem não têm trigger: são agregados que a produção edita por `updateCommands`.
...
| Procedure | Devolve |
|---|---|
| `serviceOrders.list` | OS pela data do prazo mais próximo (sem prazo por último), depois a mais nova; busca por código, dígitos, títulos dos subitens e cliente (`serviceOrderMatches`); `itemCount`, `dueOn`, `totalCents` do recebível, `shortage` quando algum subitem tem falta e as contagens de produção (`productionCount`, `startedCount`, `readyCount`) |
| `serviceOrders.get` | a OS, o cliente, o orçamento, a aprovação, a revisão aprovada (totais, custo e meta), os subitens com as reservas por variante na ordem da linha, a produção e as etapas sugeridas, o recebível e `currentFlowVersion`; "OS não encontrada" |
...
| Lista de OS (`/os`) | código, cliente e subitens, prazo com "vencido", a receber ou "sem cobrança", "aberta", "falta material" e "Produção: <resumo>" |
| Página da OS (`/os/$osId`) | cabeçalho, faixa Produção (com atrasados e bloqueio), Entrega e Financeiro, linha do fluxo de produção, um cartão por subitem com a produção ([produção](producao.md)), snapshot ("Ver medidas"), selo "bloqueado · falta <material>" e materiais (Previsto, Reservado, Falta), recebível, painel "Só para você" com custo e margem estimados e encerramento com os três itens |
...
- A reserva nunca é apagada nem regravada: a de subitem com reconciliação ativa deixa de contar em `reservedByVariant`, `reservationsOfVariant` e no `reservedMicros` de Saldos pela condição `unreleasedReservation`, e volta a contar com o estorno. `reservationsOfItems` fica sem o filtro, porque é o histórico do subitem; a falta por subitem ignora o reconciliado ([produção](producao.md)).
