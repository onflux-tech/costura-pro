...
| Valor | `consumptionPartValue`: com média (quantidade e valor positivos), o que cabe no saldo sai pela média e o excesso é provisório pela média; sem média (ponto zerado, negativo ou com valor zero ou negativo), a saída inteira é provisória pelo custo de referência da variante usada, senão zero. `valueConsumptionParts` aplica as saídas em sequência, cada uma sobre o saldo que as anteriores deixaram, no servidor e na prévia por rascunho (`reconciliationPreview`), que avisa "Fica negativo em <q>" ou "Ponto sem custo médio: <q>" |
...
- Ponto com quantidade positiva e valor negativo existe enquanto o ajuste de custo não chega (compra entrando num ponto negativo); a média dele daria saída com valor negativo, que aumentaria o estoque. Por isso a média só vale com quantidade e valor positivos.
