...
| Lote sugerido | `suggestConsumptionParts`: pontos com saldo positivo, descontado o que as linhas anteriores do rascunho já tiram, lote mais antigo por `created_at`, depois o nome do local, até 20 saídas; o que falta soma na última |
| Valor | `consumptionPartValue`: com média (quantidade positiva e valor não negativo), o que cabe no saldo sai pela média e o excesso é provisório pela média; sem média (ponto zerado, negativo ou com valor negativo), a saída inteira é provisória pelo custo de referência da variante usada, senão zero. `valueConsumptionParts` aplica as saídas em sequência, cada uma sobre o saldo que as anteriores deixaram, no servidor e na prévia por rascunho (`reconciliationPreview`), que avisa por saída "Fica negativo em <q>" ou "Ponto sem custo médio: <q>", com "Saída N · <local> · lote <rótulo>" no começo quando a linha tem mais de uma saída |
...
| Voltar e estorno | "Voltar" de `ready` mantém a reconciliação, e o avanço seguinte volta direto a `ready`; o estorno desfaz as saídas pelo mesmo valor e tira o subitem de `ready`, inclusive a reconciliação sem saída (estorno com `movementIds` vazio); material gasto a mais se registra estornando e reconciliando de novo |
...
- O rascunho da reconciliação (id, saídas com os `movementId` e o `opIdFor`) mora na página, por subitem, em funções puras de `apps/web/src/lib/reconciliation-drafts.ts`, e sobrevive a fechar e reabrir o diálogo; ele é descartado no sucesso, no `exists` e quando a leitura mostra o subitem `reconciled`, e a entrada recriada traz id e `movementId` novos, que o diálogo aberto passa a usar. Sem o descarte pela leitura, uma resposta perdida seguida de estorno fazia toda nova tentativa receber "Registro já existe". O quadro chama o mesmo `forgetSettled` com os itens dele.
...
- A quantidade da tela valida com o máximo de 6 casas, não pela precisão da variante: o previsto de um componente (0,125 m) e o saldo de um lote (0,144 m) podem ter mais casas que a precisão exibida, e a validação pela precisão recusava o próprio preenchimento.
- Ponto com quantidade positiva e valor negativo existe enquanto o ajuste de custo não chega (compra entrando num ponto negativo); a média dele daria saída com valor negativo, que aumentaria o estoque. Por isso a média só vale com quantidade positiva e valor não negativo; o brinde a R$ 0 tem média zero e sai coberto, e a referência nele inventaria um custo que nenhum ajuste corrige.
